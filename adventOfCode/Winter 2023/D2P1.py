s = '''Game 1: 1 red, 5 blue, 1 green; 16 blue, 3 red; 6 blue, 5 red; 4 red, 7 blue, 1 green
...
for index,sentence in enumerate(l):
    ...
    t_sentence = sentence[start:]
    for ind,ch in enumerate(t_sentence):
        if str.isnumeric(ch) and t_sentence[ind+1] != " ":
            num = int(ch)*10 + int(t_sentence[ind+1])
            if (num > 14) or (num > 13 and t_sentence[ind+3] == 'g') or (num > 12 and t_sentence[ind+3] == 'r'):
                exclude.append(index+1)
                break
print(5050-sum(exclude))
